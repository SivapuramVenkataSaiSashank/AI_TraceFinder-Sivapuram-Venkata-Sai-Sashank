    ...
    if uploaded_file is not None:
        ...
        for page_num, img_data in enumerate(images):
            ...
            with col2:
                ...
                if isinstance(img_data, str):
                    # It's a TIF file path, clean it up
                    if analysis_path != img_data:  # Only clean up if it's a different file
                        os.unlink(analysis_path)
                else:
                    # It's a converted file (numpy array), clean up the analysis path
                    # The analysis_path is always a temporary file in this case
                    os.unlink(analysis_path)
